...
public class CharacterQueries
{
    ...
    {
        return "INSERT INTO character (character_id, attack, lifepoints, character_name, player_id, character_class_id) VALUES ('" +
    ...
    {
        return "SELECT * FROM character WHERE character_id = " + id + ";";
    }

    public static String getCreateCharacterTableQuery()
    {
        return "CREATE TABLE character (character_id INT, attack INT, lifepoints INT, character_name CHAR(25), player_id INT," +
                " character_class_id INT, PRIMARY KEY (character_id), CONSTRAINT 'fk_player' FOREIGN KEY (player_id) " +
                "REFERENCES player(player_id) ON DELETE CASCADE ON UPDATE RESTRICT, CONSTRAINT 'fk_character_class' FOREIGN " +
